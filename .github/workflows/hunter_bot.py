...
GEMINI_CLIENTS = []
if genai:
    for k in [os.environ.get("GEMINI_API_KEY_GROUP", "")]:
...
def monitor_facebook_group(account):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(
            headless=True, 
            args=['--disable-dev-shm-usage', '--no-sandbox', '--disable-setuid-sandbox', '--disable-blink-features=AutomationControlled', '--disable-gpu']
        ) 
        ...
        context.route("**/*", lambda route: route.abort() if route.request.resource_type in ["image", "stylesheet", "media", "font", "other", "script"] else route.continue_())
...
if __name__ == "__main__":
    ...
    send_telegram("✅ GitHub Actions: Scan Complete. Sleeping until next schedule.")
